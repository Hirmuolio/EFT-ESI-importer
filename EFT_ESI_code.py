...
def import_characters():
	...
	if len(config['characters']) == 0:
		...
		for name in config['characters']:
			...
			#Import skills first			
			esi_response = esi_calling.call_esi(scope = '/v4/characters/{par}/skills', url_parameter=character_id, tokens = tokens, calltype='get', job = 'import character skills')[0]
			...
			for n in range(0, len(skills)):
				...
				try:
					...
					esi_response = esi_calling.call_esi(scope = '/v3/universe/types/{par}', url_parameter=skill_id, tokens = tokens, calltype='get', job = 'get implant name')[0]
			...
			print('')
			esi_response = esi_calling.call_esi(scope = '/v1/characters/{par}/implants', url_parameter=character_id, tokens = tokens, calltype='get', job = 'import character implants')[0]
			...
			for implant_id in implants:
				...
				try:
					...
					esi_response = esi_calling.call_esi(scope = '/v3/universe/types/{par}', url_parameter=implant_id, tokens = tokens, calltype='get', job = 'get implant name')[0]
